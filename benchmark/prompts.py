...
BENCHMARK_PROMPT_VISUAL_NAME = "maze_visual_intuition_v1"

BENCHMARK_PROMPT_VISUAL = """You are solving a visual maze puzzle from the image only.

CRITICAL INSTRUCTION: Do NOT convert the maze into a text grid, matrix, or row/column representation. Do NOT perform step-by-step BFS, DFS, or any graph-search algorithm in text. Instead, solve this the way a human would: look at the image, visually trace the walkable path from the player to the treasure with your eyes, and report what you see.
...
- Do NOT build a text representation of the grid. Reason visually, not textually.
...
Look at the maze image, visually follow the open path from start to goal, and report the shortest path you can see.
