...
\AtBeginDocument{\def\@maketitle{\vbox{\hsize\textwidth
 ...
 {\def\and{\unskip\enspace{\rmfamily and}\enspace}%
  ...
  \begin{center}
    ...
    \vspace{4pt}
    \parbox{\textwidth}{\fontsize{10pt}{12pt}\selectfont \textbf{Figure 1:} \textbf{Left:} Mazes from \textsc{MazeBench} at increasing difficulty: 5$\times$5 (A) to 20$\times$20 (X). \textbf{Right:} Solve rate vs.\ total tokens. Color = provider; size = model scale. Claude (red) clusters at 2--6\%.}
  \end{center}
  \vskip 0.08in
}}}
\makeatother
